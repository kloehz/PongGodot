...
func _on_red_team_button_pressed():
	...
	rpc("_update_players_state", players_list, players_count, players_ready)
...
func _change_red_team_remote(player_id: int, player_name: String):
	_remove_player_from_team(player_id, false)
...
func _change_blue_team_remote(player_id: int, player_name: String):
	_remove_player_from_team(player_id, true)
	var current_player = players_list.get(str(player_id))
	if current_player:
...
func peer_disconnected(id):
	if !IS_SERVER:
		return
	players_count += -1
	if players_list[str(id)].get("is_ready"):
		players_ready += -1
	print("players list: ", players_list)
	rpc("_remove_player_from_team", id, false, true)
	rpc("_update_players_state", players_list, players_count, players_ready)
...
@rpc("any_peer")
func _remove_player_from_team(id: int, is_blue_team: bool, execute_both: bool = false):
	...
	if execute_both:
		remote_blue_team(current_player)
		remove_red_team(current_player)
		return
...
func remote_blue_team(current_player):
...
func remove_red_team(current_player):
...
@rpc("any_peer")
func _update_players_state(players_list_reconcilier, players_count_remote, players_ready_remote):
	# Update players state
	players_list = players_list_reconcilier
	players_count = players_count_remote
	players_ready = players_ready_remote
